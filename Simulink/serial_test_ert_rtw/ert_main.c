...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(serial_test_M) == (NULL));
  while (runModel) {
    mw_osSignalEventWaitEver(&baserateTaskSem);
    serial_test_step();

    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(serial_test_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(serial_test_M, "stopping the model");
  runModel = 0;
}
...
int main(int argc, char **argv)
{
  ...
  hardware_init();
  rtmSetErrorStatus(serial_test_M, 0);

  /* Initialize model */
  serial_test_initialize();
